...
texture_LAI %>%
  ...
  theme_showarrows() +
    labs(yarrow = "clay (%)",
       zarrow = "silt (%)",
       xarrow = "sand(%)",
       title = "Sites by Soil Texture and Age") +
...
ggplot(master_dataset, aes(
  ...
  geom_point(size = 3) + 
  scale_color_gradient(low = "yellow", high = "red") +  # Gradient for sand
  labs(
    x = "Years", 
    y = "Average LAI", 
    color = "Sand (%)", 
...
#map
ggplot(LAI_sf) +
  geom_sf(aes(color = avg_sand, shape = water_stress_group), size = 3) +
  scale_color_viridis() + 
...
ecoregions <- st_read("path_to_your_shapefile/oregon_ecoregions.shp")
